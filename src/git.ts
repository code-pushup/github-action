  ...
  const entries = await Promise.all(
    files
      ...
      .map(async ({ file, originalFile }) => {
        ...
        return [file, { ...(originalFile && { originalFile }), lineChanges }]
      })
  )
...
function parseDiff(diff: string): LineChange[] {
  ...
  return changeSummaries
    ...
    .map((matches): LineChange => {
      ...
      return {
        prev: {
          line: parseInt(prevLine, 10),
          count: parseInt(prevAdded, 10)
        },
        curr: {
          line: parseInt(currLine, 10),
          count: parseInt(currAdded, 10)
        }
      }
    })
}
